...
- **Rate budget.** 100 req/min per (Volvo ID, client ID) AND a 10 000 req/day per-app quota. Default polling cadence is 1/min/VIN. Location is fetched (a) whenever a `state_snapshots` row is inserted — session boundaries, charging ticks — and (b) on *every* poll while the cable is disconnected, because a driving car's position changes with no Energy field changing (the Energy API often goes quiet in motion), and coupling Location to Energy changes froze the map for the whole trip. So a plugged-in/charging car still emits Location only on change, but a disconnected car emits one Location per poll: ≤1/min while moving, and ≤1 per idle interval (5 min) while parked-unplugged. If the per-app quota gets tight at scale, throttle the disconnected-car case (e.g. a separate Location heartbeat interval gated on recent movement) in `pollOne`. Every outbound Volvo HTTP call MUST go through `withRetry()` in `src/lib/volvo/retry.ts` — exp-backoff with full jitter, capped at 4 attempts, respects `Retry-After` on 429.
